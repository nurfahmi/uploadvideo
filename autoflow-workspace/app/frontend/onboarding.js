...
const onboarding = {
  ...
  render() {
    ...
    document.getElementById('onboarding-header').innerHTML = `
      ...
      <p style="text-align:center;font-size:10px;color:var(--c-fg-3);font-weight:500">${labels[current - 1]} — Step ${current} of ${total}</p>
    `;
  ...
  renderWelcome() {
    document.getElementById('onboarding-content').innerHTML = `
      <div style="text-align:center;padding:24px 0">
        ...
        <h2 style="font-size:18px;font-weight:700;color:var(--c-fg-0);margin-bottom:6px">Welcome to AUV</h2>
    ...
    document.getElementById('onboarding-footer').innerHTML = `
      <button onclick="onboarding.skip()" style="background:none;border:none;color:var(--c-fg-3);font-size:11px;cursor:pointer;font-family:inherit;font-weight:500;transition:color .1s" onmouseover="this.style.color='var(--c-fg-1)'" onmouseout="this.style.color='var(--c-fg-3)'">Skip setup</button>
  ...
  renderConnectDevice() {
    ...
    document.getElementById('onboarding-content').innerHTML = `
      <div style="text-align:center;padding:16px 0">
        ...
        <h2 style="font-size:15px;font-weight:700;color:var(--c-fg-0);margin-bottom:4px">Connect Android Phone</h2>
        <p style="font-size:11px;color:var(--c-fg-3);margin-bottom:16px">Plug in your phone via USB cable</p>
        ${hasDevice ? `
    ...
    `;
    document.getElementById('onboarding-footer').innerHTML = `
      <button onclick="onboarding.back()" style="background:none;border:none;color:var(--c-fg-3);font-size:11px;cursor:pointer;font-family:inherit" onmouseover="this.style.color='var(--c-fg-1)'" onmouseout="this.style.color='var(--c-fg-3)'">Back</button>
  ...
  async _scanDevice() {
    ...
    if (btn) btn.innerHTML = '<span style="display:flex;align-items:center;gap:6px"><svg width="14" height="14" fill="none" viewBox="0 0 24 24" style="animation:spin 1s linear infinite"><circle cx="12" cy="12" r="10" stroke="var(--c-fg-3)" stroke-width="3" fill="none" stroke-dasharray="60 30"/></svg> Scanning...</span>';
    try {
      const { invoke } = window.__TAURI__.core;
      ...
      if (devices.length > 0) {
        ...
        if (result) result.innerHTML = '<span style="color:var(--c-amber)">Phone not found.</span> <button onclick="onboarding.goTo(\'2b\')" style="color:var(--c-accent);background:none;border:none;cursor:pointer;font-family:inherit;font-size:11px;text-decoration:underline">View USB Debugging guide</button>';
  ...
  renderUSBGuide() {
    ...
    document.getElementById('onboarding-content').innerHTML = `
      <div style="padding:4px 0">
        <h2 style="font-size:15px;font-weight:700;color:var(--c-fg-0);margin-bottom:4px;text-align:center">Enable USB Debugging</h2>
        <p style="font-size:11px;color:var(--c-fg-3);margin-bottom:12px;text-align:center">Select your phone brand</p>
          ...
          ${brands.map(b => `
            <details style="background:var(--c-bg-1);border:1px solid var(--c-bg-2);border-radius:6px;overflow:hidden">
              ...
              <div style="padding:4px 12px 10px 28px;font-size:10px;color:var(--c-fg-2);line-height:1.7">
                <p>1. Open <span style="color:var(--c-accent)">Settings > About Phone</span></p>
                <p>2. ${b.note}</p>
                <p>3. Go back to <span style="color:var(--c-accent)">Developer Options</span></p>
                <p>4. Enable <span style="color:var(--c-accent)">USB Debugging</span></p>
                <p>5. Connect USB, select <span style="color:var(--c-accent)">File Transfer</span></p>
                <p>6. Tap <span style="color:var(--c-accent)">Allow</span> on the popup</p>
              </div>
            </details>
          `).join('')}
        </div>
    ...
    `;
    document.getElementById('onboarding-footer').innerHTML = `
      <button onclick="onboarding.goTo(2)" style="background:none;border:none;color:var(--c-fg-3);font-size:11px;cursor:pointer;font-family:inherit" onmouseover="this.style.color='var(--c-fg-1)'" onmouseout="this.style.color='var(--c-fg-3)'">Back</button>
    ...
    try {
      ...
      if (devices.length > 0) {
        ...
        if (!document.getElementById('ob-guide-msg')) {
          footer.insertAdjacentHTML('beforeend', '<span id="ob-guide-msg" style="font-size:10px;color:var(--c-amber);position:absolute;left:50%;transform:translateX(-50%)">Phone still not detected</span>');
  ...
  renderPlatform() {
    ...
    document.getElementById('onboarding-content').innerHTML = `
      <div style="text-align:center;padding:16px 0">
        <h2 style="font-size:15px;font-weight:700;color:var(--c-fg-0);margin-bottom:4px">Choose Platform</h2>
        <p style="font-size:11px;color:var(--c-fg-3);margin-bottom:16px">Select one or both</p>
  ...
  renderFirstUpload() {
    document.getElementById('onboarding-content').innerHTML = `
      <div style="padding:4px 0">
        <h2 style="font-size:15px;font-weight:700;color:var(--c-fg-0);margin-bottom:4px;text-align:center">First Video Upload</h2>
        <p style="font-size:11px;color:var(--c-fg-3);margin-bottom:12px;text-align:center">Try uploading one video to verify everything works</p>
    ...
    document.getElementById('onboarding-footer').innerHTML = `
      ...
      <div style="display:flex;gap:8px">
        <button onclick="onboarding.next()" style="background:none;border:none;color:var(--c-fg-3);font-size:11px;cursor:pointer;font-family:inherit" onmouseover="this.style.color='var(--c-fg-1)'" onmouseout="this.style.color='var(--c-fg-3)'">Skip</button>
  ...
  async _startUpload() {
    if (!this._uploadFlow || !this.data.deviceId) {
      if (!this.data.deviceId) {
        ...
        c.innerHTML = '<p style="color:var(--c-amber)">No device connected. Skip this step or go back.</p>';
      }
      return;
    }
    ...
    if (missing.length) {
      const c = document.getElementById('ob-upload-console');
      c.style.display = 'block';
      c.innerHTML = `<p style="color:var(--c-amber)">Fill in: ${missing.map(f => f.label).join(', ')}</p>`;
      return;
    }
  ...
  _onUploadSuccess() {
    ...
    el.innerHTML = `
      ...
      <p style="font-size:13px;font-weight:600;color:var(--c-green)">Upload successful!</p>
    `;
    document.getElementById('onboarding-footer').innerHTML = `
      <div></div>
  ...
  renderComplete() {
    document.getElementById('onboarding-content').innerHTML = `
      <div style="text-align:center;padding:24px 0">
        ...
        <h2 style="font-size:18px;font-weight:700;color:var(--c-fg-0);margin-bottom:6px">Setup Complete!</h2>
        <p style="font-size:12px;color:var(--c-fg-2);margin-bottom:20px">You're ready to start uploading videos</p>
        <div style="text-align:left;background:var(--c-bg-1);border:1px solid var(--c-bg-2);border-radius:8px;padding:14px;display:flex;flex-direction:column;gap:8px">
          <div style="display:flex;align-items:center;gap:10px;font-size:11px">
            ...
            <span style="color:var(--c-fg-1)"><span style="color:var(--c-fg-0);font-weight:600">Devices</span> — connect & manage your phones</span>
          </div>
          <div style="display:flex;align-items:center;gap:10px;font-size:11px">
            ...
            <span style="color:var(--c-fg-1)"><span style="color:var(--c-fg-0);font-weight:600">Upload Queue</span> — add videos or import CSV</span>
          </div>
          <div style="display:flex;align-items:center;gap:10px;font-size:11px">
            <svg width="16" height="16" fill="none" stroke="var(--c-accent)" stroke-width="1.5" viewBox="0 0 24 24"><path d="M9 3h6l2 2v4l-2 2H9L7 9V5l2-2z"/></svg>
            <span style="color:var(--c-fg-1)"><span style="color:var(--c-fg-0);font-weight:600">Flow Action</span> — customize automation steps</span>
          </div>
          <div style="display:flex;align-items:center;gap:10px;font-size:11px">
            <svg width="16" height="16" fill="none" stroke="var(--c-green)" stroke-width="2" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
            <span style="color:var(--c-fg-1)">Click <span style="color:var(--c-green);font-weight:600">Run Flow</span> to begin uploading</span>
          </div>
        </div>
      </div>
    `;
