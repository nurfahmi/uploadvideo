...
const NAV_GROUPS = [
  {
    label: 'Workspace',
    items: [
      { id: 'dashboard', label: 'Dashboard', color: '#58a6ff', icon: '<path d="M4 5a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1V5zm10 0a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 15a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1v-4zm10 0a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z"/>' },
      { id: 'devices', label: 'Devices', color: '#39d2c0', icon: '<rect x="7" y="2" width="10" height="20" rx="2"/><path d="M12 18h.01"/>' },
      { id: 'queue', label: 'Upload Queue', color: '#d29922', icon: '<path d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>' },
    ],
  },
  {
    label: 'Tools',
    items: [
      { id: 'editor', label: 'Flow Action', color: '#bc8cff', icon: '<path d="M9 3h6l2 2v4l-2 2H9L7 9V5l2-2zM7 13h4l2 2v4l-2 2H7l-2-2v-4l2-2zm6 0h4l2 2v4l-2 2h-4l-2-2v-4l2-2z"/><path d="M12 7v6m-2 4h-1m7-4h-1" stroke-dasharray="2 2"/>' },
      { id: 'history', label: 'History', color: '#58a6ff', icon: '<path d="M12 8v4l3 3"/><circle cx="12" cy="12" r="9"/><path d="M3 12h1m16 0h1M12 3v1m0 16v1"/>' },
      { id: 'console', label: 'Console', color: '#30363d', icon: '<path stroke-linecap="round" stroke-linejoin="round" d="M6.75 7.5l3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0021 18V6a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 6v12a2.25 2.25 0 002.25 2.25z"/>' },
...
export function renderSidebar() {
  ...
  sb.innerHTML = `
    ...
    <div style="display:flex;${c ? 'flex-direction:column;align-items:center;gap:4px' : 'align-items:center;gap:7px'};padding:10px 10px 6px">
      <div style="width:${c ? '30' : '26'}px;height:${c ? '30' : '26'}px;border-radius:6px;background:linear-gradient(135deg,#58a6ff,#bc8cff);display:flex;align-items:center;justify-content:center;flex-shrink:0">
    ...
    <nav style="flex:1;padding:8px 6px;display:flex;flex-direction:column;gap:12px;overflow-y:auto">
      ${NAV_GROUPS.map(group => {
              ...
              ${group.items.map(n => {
                ...
                return `
                  <div class="sb-item ${isActive ? 'sb-active' : ''}" data-nav="${n.id}" style="display:flex;align-items:center;justify-content:center;padding:7px;border-radius:6px;cursor:pointer;position:relative;transition:background .1s">
                    ...
                    <span class="nav-tooltip">${n.label}</span>
                  </div>`;
              }).join('')}
            </div>`;
        }
        ...
        return `
          <div>
            <p style="font-size:9px;font-weight:600;color:#484f58;text-transform:uppercase;letter-spacing:.5px;padding:0 8px;margin-bottom:4px">${group.label}</p>
              ...
              ${group.items.map(n => {
                ...
                return `
                  <div class="sb-item ${isActive ? 'sb-active' : ''}" data-nav="${n.id}" style="display:flex;align-items:center;gap:10px;padding:7px 10px;border-radius:6px;cursor:pointer;transition:background .1s">
                    ...
                    <span style="font-size:12px;font-weight:${isActive ? '600' : '500'};color:${isActive ? '#f0f6fc' : '#8b949e'};transition:color .1s">${n.label}</span>
                  </div>`;
              }).join('')}
            </div>
          </div>`;
